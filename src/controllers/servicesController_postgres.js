...
const getServices = async (req, res) => {
  try {
    const db = getDB();
    const result = await db.query('SELECT * FROM servicios ORDER BY created_at DESC');
...
const updateService = async (req, res) => {
  try {
    const { id } = req.params;
    const { workOrder, plate, serviceType, description, cost, status } = req.body;
    const db = getDB();
...
const deleteService = async (req, res) => {
  try {
    ...
    const result = await db.query('DELETE FROM servicios WHERE id = $1 RETURNING *', [id]);
...
const getServiceStats = async (req, res) => {
  try {
    ...
    const totalResult = await db.query('SELECT COUNT(*) as total FROM servicios');
    const statusResult = await db.query('SELECT status, COUNT(*) as count FROM servicios GROUP BY status');
