...
const createCar = async (req, res) => {
  try {
    const { plate, brand, model, year, ownerName, ownerPhone, ownerEmail, notes } = req.body;
    const db = getDB();
...
const getCars = async (req, res) => {
  try {
    const db = getDB();
    const result = await db.query('SELECT * FROM vehiculos ORDER BY created_at DESC');
...
const getCarById = async (req, res) => {
  try {
    ...
    const result = await db.query('SELECT * FROM vehiculos WHERE id = $1', [id]);
...
const getCarByPlate = async (req, res) => {
  try {
    ...
    const result = await db.query('SELECT * FROM vehiculos WHERE plate = $1', [plate]);
...
const deleteCar = async (req, res) => {
  try {
    ...
    const result = await db.query('DELETE FROM vehiculos WHERE id = $1 RETURNING *', [id]);
